...
async function variants(url) {
  ...
  const widths = [
    ...[480, 800, 1200, 1600].filter((value) => value < width),
    width,
  ];
  const stem = url.replace(/\.\w+$/, '');
  const outputs = await Promise.all(
    widths.map(async (value) => {
  ...
  return {
    ...
    srcset:
      outputs.length > 1
        ? outputs.map((item) => `${item.url} ${item.width}w`).join(', ')
        : '',
    width,
    height,
  };
...
const urls = new Set(
  pages.flatMap(({ html }) => html.match(imagePattern) || []),
);
const images = new Map(
  await Promise.all([...urls].map(async (url) => [url, await variants(url)])),
);
      ...
      if (image.srcset) {
        ...
        if (!attr(node, 'sizes'))
          setAttr(
            node,
            'sizes',
            '(max-width: 700px) 100vw, (max-width: 1100px) 80vw, 960px',
          );
      }
