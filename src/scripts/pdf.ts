...
import { qs, qsa, on, delegate } from './dom.ts';

function normalizePdfUrl(url: string | null): string {
...
function buildInlinePdfPreviewUrl(url: string | null): string {
...
export function setupPdfPreviews() {
  ...
  const pumpQueue = () => {
    while (activeLoads < maxConcurrentLoads && queue.length) {
      ...
      if (!card || card.getAttribute('data-pdf-preview-state') === 'loaded')
        continue;
  ...
  const requestPreviewLoad = (card: HTMLElement) => {
  ...
  cards.forEach((card) => {
    const pdfUrl = normalizePdfUrl(
      card.getAttribute('data-pdf') || card.getAttribute('href'),
    );
  ...
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        requestPreviewLoad(entry.target as HTMLElement);
...
export function setupPdfModal() {
  ...
  delegate('click', '.pdf-preview', (event, link) => {
    ...
    if (prefersNativePdfOpen()) {
      if (!window.open(pdfUrl, '_blank', 'noopener'))
    ...
    title.textContent =
      link.getAttribute('data-title') || link.textContent.trim();
